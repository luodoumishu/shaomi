...
<li>
	<a href="sq_index.jsp?orgCode=<%=orgCode %>&orgName=<%=orgName %>" id="shouye" class="hove">首页</a>
</li>
			<%
	        ...
	        CmsChannel cc = (CmsChannel)iCmsChannelService.findByChlCode("sq_daohang");
            ...
            List<CmsChannelItem> setChlItem = cmsChannelItemService.findAllItem(cc.getId());
   			...
   			for(int index_menu= 0;index_menu < length;index_menu++){
   					CmsChannelItem ccItem = (CmsChannelItem)setChlItem.get(index_menu);
   	   				...
   	   				if(cm.getMenuType() == 0){
   	   					//真实栏目
  						menuUrl = basePath + "web/sq/menu.jsp"+"?menuName="+cm.getMenuName()+"&menuId="+itemId.toString()+"&orgCode="+orgCode+"&orgName="+orgName;
