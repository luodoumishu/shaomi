...
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
	        ...
	        CmsChannel cc = (CmsChannel)iCmsChannelService.findByChlCode("menu_left_daohang");
            ...
            List<CmsChannelItem> setChlItem = cmsChannelItemService.findAllItem(cc.getId());
   			...
   			for(int index_menu= 0;index_menu<setChlItem.size();index_menu++){
   					CmsChannelItem ccItem = (CmsChannelItem)setChlItem.get(index_menu);
   	   				...
   	   				if(cm.getMenuType() == 0){
   	   					//真实栏目
   	   					menuUrl = basePath + "web/default/menu.jsp"+"?menuName="+cm.getMenuName()+"&menuId="+itemId.toString()+"&orgCode="+orgCode+"&orgName="+orgName;
   	   		  ...
   	   		  %>
<a href="<%=menuUrl %>" id="<%=cm_id%>" class="hove"><%=cm.getMenuName()%></a>
			<%
   	   		}else{
   	   		  %>
<a href="<%=menuUrl %>" id="<%=cm_id%>"><%=cm.getMenuName()%></a>
			<%
   	   		}
   				}
   	   		%>