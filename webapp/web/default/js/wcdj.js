	    	...
	    	if(null != result && null != result.resultData){
	    		if(result.resultCode == 0){
	    			...
	    			for(var i = 0; i<datas.length;i++){
	    				...
	    				var zxValue = org_name.substring(org_name.length-1,org_name.length);
	    				org_name = encodeURIComponent(data.name);
	    				if(zxValue == "镇" || zxValue == "乡" || zxValue == "场"){
	    						html +="<li><a href='"+path+"/web/z/z_index.jsp?orgName="+org_name+"&orgCode="+data.pyCode+"' target='_blank' >"+data.name+"</a></li>";
	    				}
	    			}
	    			$("#xzzl").html(html);
	    		}else{
	    	...
	    	if(null != result && null != result.resultData){
	    		if(result.resultCode == 0){
	    			...
	    			for(var i = 0; i<datas.length;i++){
	    				...
	    				org_name = encodeURIComponent(data.name);
	    				if(zxValue != "镇" && zxValue !="乡" && zxValue != "场"){
	    					html +="<li><a href='"+path+"/web/jg/jg_index.jsp?orgName="+org_name+"&orgCode="+data.pyCode+"' target='_blank'>"+data.name+"</a></li>";
	    				}
	    			}
	    			html += ""
	    			$("#jgzl").html(html);
