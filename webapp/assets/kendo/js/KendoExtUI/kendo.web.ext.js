...
(function (kendo, $) {
    ...
    //Add By Wuk 2013-11-27 增加详细信息窗口
    /*
    *
    * ExtDetailDialog
    *
    */
    ...
    var ExtDetailDialog = kendo.ui.Window.extend({
        _buttonTemplate: kendo.template('<div class="k-ext-dialog-buttons" style="position:absolute; bottom:10px; text-align:center; width:#= parseInt(width) - 14 #px;"><div style="display:inline-block"># $.each (buttons, function (idx, button) { # <button class="k-button" style="margin-right:5px; width:100px;">#= button.name #</button> # }) # </div></div>'),
        _contentTemplate: kendo.template('<div class="k-ext-dialog-content" style="height:#= parseInt(height) - 55 #px;; width:#= parseInt(width) - 14 #px;overflow:auto;">'),
        _detailTemplate: kendo.template('<div class="k-ext-dialog-detail" style="width:#= parseInt(width) #px;"><ul id="extDetailAlert"><li>详细错误信息<div style="word-wrap: break-word; word-break: normal;">#= detailMessage #</div></li></ul></div>'),
        ...
        init: function (element, options) {
            ...
            $(element).after(that._detailTemplate(options));
            $(element).after(that._buttonTemplate(options));

            $("#extDetailAlert").kendoPanelBar({
            });
        ...
        options: {
            name: "ExtDetailDialog"
        }
    });
    kendo.ui.plugin(ExtDetailDialog);
    ...
    kendo.ui.ExtAlertDialog = {
        show: function (options) {
            return new $.Deferred(function (deferred) {
                ...
                $(document.body).append(kendo.format("<div id='extAlertDialog' style='position:relative;'><div style='position:absolute;left:10px;top:10px;' class='{0}'></div><div style='display:inline-block;margin-left:45px;padding-top:13px;'>{1}</div></div>", options.icon, options.message));
    ...
    //Add By Wuk 2013-11-27 增加详细错误窗口
    /*
     *
     * DetailAlertDialog
     *
    ...
    kendo.ui.ExtDetailAlertDialog = {
        show: function (options) {
            return new $.Deferred(function (deferred) {
                var dialog = null;

                if ($("#extDetailAlertDialog").length > 0) {
                    $("#extDetailAlertDialog").parent().remove();
                }
                ...
                $(document.body).append(kendo.format("<div id='extDetailAlertDialog' style='position:relative;'><div style='position:absolute;left:10px;top:10px;' class='{0}'></div><div style='display:inline-block;margin-left:45px;padding-top:13px;'>{1}</div></div>", options.icon, options.message));
                dialog = $("#extDetailAlertDialog").kendoExtDetailDialog(options).data("kendoExtDetailDialog");
                $("#extDetailAlertDialog").parent().find("div.k-window-titlebar div.k-window-actions").empty();
                dialog.center().open();
            });
        },

        hide: function () {
            $("#extDetailAlertDialog").data("kendoExtDetailDialog").close();
        }
    };
    ...
    kendo.ui.ExtOkCancelDialog = {
        show: function (options) {
            return new $.Deferred(function (deferred) {
                ...
                options = $.extend({
                    width: options.Width === undefined ? "300px;" : options.Width,
                    height: options.Height === undefined ? "100px;" : options.Height,
                    buttons: [{
                        name: "确定",
                        click: function (e) {
                            $("#extOkCancelDialog").data("kendoExtDialog").close();
                            deferred.resolve({ button: "确定" });
                        }
                    }, {
                        name: "取消",
                        click: function (e) {
                            $("#extOkCancelDialog").data("kendoExtDialog").close();
                            deferred.resolve({ button: "取消" });
                        }
                    }],
                    modal: true,
                    visible: false,
    ...
    var ExtDropDownTreeView = kendo.ui.Widget.extend({
        ...
        init: function (element, options) {
            ...
            $(element).append(kendo.format("<input id='extDropDown{0}' class='k-ext-dropdown'/>", that._uid));
            $(element).append(kendo.format("<div id='extTreeView{0}' class='k-ext-treeview' style='z-index:15000;'/>", that._uid));
            ...
            var dropdownWidth = $dropdownRootElem.width();

            $dropdownRootElem.width($dropdownRootElem.parent().width() - 3);
            
            var diyWidth = $dropdownRootElem.width() - 3;
            
            var diyHeight = "";
            if(options.treeview && options.treeview.width){
            	diyWidth = options.treeview.width;
            }
            
            if(options.treeview && options.treeview.height){
            	diyHeight = options.treeview.height;
            }
            // Hide the treeview.
            $treeviewRootElem
                .width(diyWidth)
        ...
        enable: function (enable) {
            this._dropdown.enable(enable);
            this._treeview.enable(enable);
        },
