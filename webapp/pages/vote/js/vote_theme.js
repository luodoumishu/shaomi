...
xjj.cms.vote = (function(){
	...
	var ToolbarHtml = new CreateToolBarButton({
        GridId: "grid",
        Url: path+"/pages/vote/vote_theme_edit.jsp",
        Add: true,
        Edit: true,
        Del: true
 	});
	...
	var init_listPage = function(){
	  ...
	  // 列表表格
	        grid = CreateGrid({
		        ...
		        Filterable: false,    
		        Columns: [ 
		                  { field: "themeName",width: "25%",title: "投票主题"},
		                  { field: "showCode",width: "10%",title: "显示代码"},
		                  { field: "isValid",width: "10%",title: "是否有效",values: [{text: "有效",value: 0 
	           		   		},{text: "无效", value: 1}]},
		                  { field: "voteTotal",width: "10%",title: "总投票数"},
		                  { field: "sort",width: "10%",title: "排序"},
		                  {width: "25%",title: "操作",  command:{text: "设置投票项", click: themeSetting}}
		       ]
	         });
	};
	/******编辑页面js******/
	...
	var  checkForm =  function () {
	  ...
	  var initForm = function (){
		 ...
		 if (editType == "add") {//VIEW_MODEL层的数据初始化
			 item.sort = 0;
			 item.isValid = 0;
//			 item.is_same_ip_vote = 0;
//			 item.is_same_mac_vote = 0;
//			 item.is_need_person = 0;
		 }
...
var  onDelete = function () {
 ...
 var del = function () {
     ...
     if (typeof (data) != undefined) {
     	AjaxDoPost(path+"/cms/vote/theme/delete.json","models=" + JSON.stringify(data),true,xjj.cms.vote.deleteSuccess,xjj.cms.vote.deleteError);
     }
 };
 function themeSetting() {
	...
	selectRowIndex = row.index();
	Item = grid.dataItem(row);	
	location.href = path+"/pages/vote/vote_theme_setting.jsp?themeName=" + Item.themeName +"&themeId=" + Item.id;
}